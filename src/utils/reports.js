...
export const generateVehiclePDF = (vehicle, outgoings, totalCost) => {
  const doc = new jsPDF();
  ...
  doc.text("Detalles / Falla:", 14, 64);
  doc.setFont("Helvetica", "oblique");
  const splitDetails = doc.splitTextToSize(vehicle.details || "Sin detalles registrados.", 175);
  doc.text(splitDetails, 14, 69);
  
  const nextY = 69 + (splitDetails.length * 5) + 5;
  doc.setDrawColor(226, 232, 240);
  ...
  const tableData = outgoings.map(out => {
    ...
    return [
      formatDate(out.date).split(' ')[0], // just date
      out.materialName,
      qty,
      formatCurrency(cost),
